...
import { FaRegEdit, FaStar, FaStarHalfAlt } from "react-icons/fa";
...
const ViewBookDetails = () => {
  ...
  const addToFavourite = async () => {
    try {
      const response = await axios.put(
        "https://bookverse-tpi0.onrender.com/api/v1/add-to-favourite",
        {},
        { headers }
      );
  ...
  const addToCart = async () => {
    try {
      const response = await axios.put(
        "https://bookverse-tpi0.onrender.com/api/v1/add-to-cart",
        {},
        { headers }
      );
      alert(response.data.message);
  ...
  const deleteBook = async () => {
    try {
      const response = await axios.delete(
        "https://bookverse-tpi0.onrender.com/api/v1/delete-book",
        { headers }
      );
      alert(response.data.message);
      history("/all-books");
